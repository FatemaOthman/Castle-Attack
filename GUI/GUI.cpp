...
void GUI::PrintMessage(string msg, int x) const	//Prints a message on status bar
{
	//ClearStatusBar();	//First clear the status bar

	pWind->SetPen(DARKRED);
	pWind->SetFont(18, BOLD, BY_NAME, "Arial");
	pWind->DrawString(10, (WindHeight - (int)(StatusBarHeight / 1.05) + (14 * x)), msg);
...
void GUI::DrawMenu() const
{
	...
	MenuItemImages[MENU_STEP] = "images\\MenuItems\\StepByStep.jpg";
	MenuItemImages[MENU_IACTIVE] = "images\\MenuItems\\InterActive.jpg";
	MenuItemImages[MENU_SILENT] = "images\\MenuItems\\Skull.jpg";
	MenuItemImages[MENU_EXIT] = "images\\MenuItems\\Key.jpg";
...
{
	int L = CastleWidth / 2;
	pWind->DrawImage("images\\MenuItems\\Grass_double1.jpg", 0, MenuBarHeight, WindWidth / 2, (WindHeight / 2) - 2 * MenuBarHeight);
	pWind->DrawImage("images\\MenuItems\\Minecraft_snow.jpg", 0, (WindHeight / 2) - MenuBarHeight, WindWidth / 2, (WindHeight / 2) - 2 * MenuBarHeight);

	pWind->DrawImage("images\\MenuItems\\SSEA.jpg", WindWidth / 2, MenuBarHeight, WindWidth / 2, (WindHeight / 2) - 2 * MenuBarHeight);
	pWind->DrawImage("images\\MenuItems\\sand.jpg", WindWidth / 2, (WindHeight / 2) - MenuBarHeight, WindWidth / 2, (WindHeight / 2) - 2 * MenuBarHeight);
	...
	pWind->DrawImage("images\\MenuItems\\Castle11.jpg", CastleStartX, CastleStartY, CastleEndX - CastleStartX, CastleEndY - CastleStartY);
	...
	for(int region = A_REG; region <= D_REG; region++)
	{
		...
		else // print message maximum reached in this region
		{
			PrintMessage("Cannot Draw Enemies of region " + to_string(region+1) + 
				"! More than" + to_string(MaxVerticaEnemies) + " enemies are in the same region and distance..",1);
			Sleep(1000); // because may be more than one region has drawing overflow
		}
	}
}

MenuAction GUI::GetUserAction()
{
	...
	if (y >= 0 && y < MenuBarHeight)
	{
		...
		{
		case MENU_IACTIVE: return IACTIVE;
			break;
		case MENU_SILENT: return SILENT;
			break;
...
void GUI::DrawUnPaved(int p1, int p2, int p3, int p4)
{
	pWind->DrawImage("images\\MenuItems\\ForestPaved.jpg", WindWidth / 2 - (p1 * 8 - EnemyWidth + CastleWidth / 2), MenuBarHeight, EnemyWidth * 2, (WindHeight / 2 - StatusBarHeight) + MenuBarHeight);
	pWind->DrawImage("images\\MenuItems\\DesertPaved.jpg", WindWidth / 2 + (p3 * 8 - EnemyWidth + CastleWidth / 2), YHalfBattleArea, -(EnemyWidth * 2), (WindHeight / 2 - StatusBarHeight) + MenuBarHeight);
	pWind->DrawImage("images\\MenuItems\\SnowPaved.jpg", WindWidth / 2 - (p4 * 8 - EnemyWidth + CastleWidth / 2), BattleAreaHeight / 2 + MenuBarHeight, EnemyWidth * 2, (WindHeight / 2 - StatusBarHeight) + MenuBarHeight);
	pWind->DrawImage("images\\MenuItems\\SeaPaved.jpg", WindWidth / 2 + (p2 * 8 - EnemyWidth + CastleWidth / 2), MenuBarHeight, -(EnemyWidth * 2), (WindHeight / 2 - StatusBarHeight) + MenuBarHeight);

	/*pWind->DrawLine(WindWidth / 2 - (p1 * 8 - EnemyWidth + CastleWidth / 2), YHalfBattleArea, WindWidth / 2 - (p1 * 8 - EnemyWidth + CastleWidth / 2), MenuBarHeight);
	pWind->DrawLine(WindWidth / 2 + (p2 * 8 - EnemyWidth + CastleWidth / 2), YHalfBattleArea, WindWidth / 2 + (p2 * 8 - EnemyWidth + CastleWidth / 2), MenuBarHeight);
	pWind->DrawLine(WindWidth / 2 - (p3 * 8 - EnemyWidth + CastleWidth / 2), YHalfBattleArea, WindWidth / 2 - (p3 * 8 - EnemyWidth + CastleWidth / 2), (2 * YHalfBattleArea - MenuBarHeight));
	pWind->DrawLine(WindWidth / 2 + (p4 * 8 - EnemyWidth + CastleWidth / 2), YHalfBattleArea, WindWidth / 2 + (p4 * 8 - EnemyWidth + CastleWidth / 2), (2 * YHalfBattleArea - MenuBarHeight));*/
